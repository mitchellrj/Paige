(function() {
  ...
  ensure_directory = function(dir, callback) {
    var mkdirp, sep;
    mkdirp = function(base, path_stack, callback) {
      var d, p;
      if (!path_stack.length) {
        return callback();
      }
      d = path_stack.shift();
      p = path.join(base, d);
      return path.exists(p, function(exists) {
        if (exists) {
          return fs.stat(p, function(error, stat) {
            if (error != null) {
              console.log("Couldn't stat " + p + ".");
            }
            if (!stat.isDirectory()) {
              console.log("" + p + " exists but is not a directory.");
            }
            if (stat.isDirectory()) {
              return mkdirp(p, path_stack, callback);
            }
          });
        } else {
          return fs.mkdir(p, 0755, function(error) {
            if (error != null) {
              console.log("Could not create directory " + p + ".");
            }
            if (error == null) {
              return mkdirp(p, path_stack, callback);
            }
    ...
    sep = path.join('a', 'b')[1];
    return mkdirp(path.normalize('.'), dir.split(sep), callback);
  };
  copy_image = function() {
    var desired_image;
